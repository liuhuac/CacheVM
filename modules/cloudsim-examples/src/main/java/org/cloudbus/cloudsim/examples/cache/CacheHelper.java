...
public class CacheHelper {
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			int vmType = i / (int) Math.ceil((double) vmsNumber / ExpConstants.VM_TYPES);
			vms.add(new PowerVm(
					i,
					brokerId,
					ExpConstants.VM_MIPS[vmType],
					ExpConstants.VM_PES[vmType],
					ExpConstants.VM_RAM[vmType],
					ExpConstants.VM_BW,
					ExpConstants.VM_SIZE,
					1,
					"Xen",
					new CloudletSchedulerDynamicWorkload(ExpConstants.VM_MIPS[vmType], ExpConstants.VM_PES[vmType]),
					ExpConstants.SCHEDULING_INTERVAL));
		}
		return vms;
	}
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			int hostType = i % ExpConstants.HOST_TYPES;

			List<Pe> peList = new ArrayList<Pe>();
			for (int j = 0; j < ExpConstants.HOST_PES[hostType]; j++) {
				peList.add(new Pe(j, new PeProvisionerSimple(ExpConstants.HOST_MIPS[hostType])));
			}

			hostList.add(new PowerHostUtilizationHistory(
					i,
					new RamProvisionerSimple(ExpConstants.HOST_RAM[hostType]),
					new BwProvisionerSimple(ExpConstants.HOST_BW),
					ExpConstants.HOST_STORAGE,
					peList,
					new VmSchedulerTimeSharedOverSubscription(peList),
					ExpConstants.HOST_POWER[hostType]));
		}
		return hostList;
	}
	...
	public static List<Cloudlet> createCloudletList(int brokerId, int cloudletsNumber) {
		...
		long fileSize = 300;
		long outputSize = 300;
		long seed = ExpConstants.CLOUDLET_UTILIZATION_SEED;
		...
		for (int i = 0; i < cloudletsNumber; i++) {
			...
			if (seed == -1) {
				cloudlet = new Cloudlet(
						i,
						ExpConstants.CLOUDLET_LENGTH,
						ExpConstants.CLOUDLET_PES,
						fileSize,
						outputSize,
